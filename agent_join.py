...
vlm_config = get_vlm_config(default_model="qwen-vl-max")
...
def ask_vlm_for_action(task, screenshot_path):
    """
    终极中枢大脑：绕过受限的 CLI，直接通过 HTTP 与 VLM 视觉大模型进行高速通信！
    """
    ...
    payload = {
        ...
        "max_tokens": 300,
        "temperature": 0.0 # 保持 0，我们需要稳定的 JSON 输出，不需要创造力
    }

    try:
        response = requests.post(API_URL, headers=headers, json=payload, timeout=40)
        response.raise_for_status() # 检查 HTTP 错误 (如 401, 404, 413)
        ...
        print(f"❌ [API 通信错误]: {e}")
        # 如果报错 400 或 404，通常是 MODEL_NAME 不对或该模型不支持视觉
...
def run_integration_test(playwright):
    ...
    task_instruction = "在页面的搜索框中输入 '流浪地球'，然后按回车键进行搜索。如果进入了搜索结果页面，说明任务成功，请输出 stop。"
    print(f"📋 [评测台] 当前任务: {task_instruction}")
    
    print("🌐 [评测台] 正在加载豆瓣首页...")
    try:
        page.goto("https://movie.douban.com/explore", wait_until="load", timeout=30000)
    except Exception as e:
    ...
    for step in range(1, 6):
        ...
        if action_type == "stop":
            ...
            try:
                # 【终极防漏策略】：增加 #inp-query 专属定位，且必须是 visible=true (肉眼可见)
                search_box = page.locator('input[name="q"], #inp-query, input[type="text"]').locator('visible=true').first
                
                # 很多现代网站需要先点击激活输入框，才能打字
